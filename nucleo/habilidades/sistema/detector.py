...
def es_busqueda(texto, codigo_adjunto=""):
    ...
    if hay_url(texto) or codigo_adjunto:   # URLs y código son de otras habilidades
        return False
    # Búsqueda de ARCHIVOS/carpetas LOCALES → es de la habilidad 'sistema', no de la web.
    _LOCAL = ("archivo", "archivos", "carpeta", "en mi pc", "en el pc", "en mi disco",
              "en mi computadora", "en mi compu", "en mi máquina", "en mi maquina", "en disco")
    _INTERNET = ("internet", "online", "web", "google", "en línea", "en linea")
    if any(l in t for l in _LOCAL) and not any(i in t for i in _INTERNET):
        return False
